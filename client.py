...
def display_response(response, showBDCSources=False):
    ...
    if "guardrail_response" in keys:
        st.markdown(f"{get_response_text(response, "guardrail_response", "Missing guardrail response")}", help="This response was generated by a guardrail as the user prompt was out of scope")
        return
    if "predefined_response" in keys:
        for predefined in response.get("predefined_response", []):
            st.markdown(predefined, help="This is a predefined response to meet strict NHLBI guidelines")
